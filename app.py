    ...
    try:
        ...
        if len(prophet_data) >= 36:  # Need enough data for CV
            try:
                ...
                st.warning("Prophet diagnostics not available. Using default parameters.")
        else:
        ...
        # Default params if optimization failed
        if not best_params:
            best_params = {
                'changepoint_prior_scale': 0.05,
                'seasonality_prior_scale': 10.0,
        ...
        return forecast_values, best_mape if best_mape != np.inf else 100.0
    ...
    try:
        ...
        if best_model is not None:
            ...
            try:
                ...
                forecast_std = np.std(best_model.resid)
